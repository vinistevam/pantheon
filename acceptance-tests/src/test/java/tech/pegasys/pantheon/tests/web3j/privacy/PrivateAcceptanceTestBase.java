...
public class PrivateAcceptanceTestBase extends AcceptanceTestBase {
  ...
  String getExecuteStoreFuncCluster() {
    ...
    PrivateTransaction pTx =
        privateTx.storeFunctionTransaction(1, to, from, privateFrom, privateFor, keypair);
    return RLP.encode(pTx::writeTo).toString();
  }
        ...
        SECP256K1.KeyPair.create(
            SECP256K1.PrivateKey.create(
                new BigInteger(
                    "c87509a1c067bbde78beb793e6fa76530b6382a4c0241e5e4a9ec0a0f44dc0d3", 16)));
    PrivateTransaction pTx =
        privateTx.storeFunctionTransaction(0, to, from, privateFrom, privateFor, keypair);
    return RLP.encode(pTx::writeTo).toString();
  }
